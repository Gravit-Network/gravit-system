...
function App() {
  ...
  const generateHypothesis = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.detail || "Failed to generate hypothesis");
      }
      setHypothesis({
        id: data.hypothesis_id,
        text: data.hypothesis,
        confidence: Number(data.confidence ?? 0),
        provenance: data.provenance ?? llm,
  ...
  const runConsensus = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.detail || "Failed to run consensus");
      }
      const c = data.consensus || {};
      setConsensus({
        converged: Boolean(c.converged),
        iterations: c.iterations ?? 0,
        finalDivergence: c.finalDivergence ?? c.final_divergence ?? 0,
        consensusTimeMs: c.consensusTimeMs ?? c.consensus_time_ms ?? 0,
        fixedPoint: c.fixedPoint ?? c.fixed_point ?? [],
      });
    } catch (error) {
  ...
  const executeEQL = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.detail || "Failed to execute EQL query");
      }
      ...
      <main className="app-main">
        <section className="card">
          ...
          {hypothesis && (
            <div className="result">
              ...
              <p>
                <strong>ID:</strong> {hypothesis.id}
              </p>
              <p>
                <strong>Text:</strong> {hypothesis.text}
              </p>
              <p>
                <strong>Confidence:</strong> {(hypothesis.confidence * 100).toFixed(1)}%
              </p>
              <p>
                <strong>Provenance:</strong> {hypothesis.provenance}
              </p>
        ...
        {consensus && (
          <section className="card">
            ...
            <div className="result">
              <p>
                <strong>Converged:</strong> {consensus.converged ? "Yes" : "No"}
              </p>
              <p>
                <strong>Iterations:</strong> {consensus.iterations}
              </p>
              <p>
                <strong>Final KL-Divergence:</strong> {consensus.finalDivergence.toExponential(3)}
              </p>
              <p>
                <strong>Time:</strong> {consensus.consensusTimeMs.toFixed(2)} ms
              </p>
              <p>
                <strong>Fixed Point Distribution:</strong>
              </p>
        ...
        <section className="card">
          ...
          {queryResults.length > 0 && (
            <div className="result">
              ...
              <pre className="query-results">{JSON.stringify(queryResults, null, 2)}</pre>
            </div>
          )}
        </section>
      </main>
